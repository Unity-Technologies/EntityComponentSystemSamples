...
public class SamplesSelector : EditorWindow, IHasCustomMenu
{
    ...
    [MenuItem("Samples/Samples")]
    public static void ShowExample()
    {
        SamplesSelector wnd = GetWindow<SamplesSelector>();
    ...
    public void CreateGUI()
    {
        ...
        label = new Label($"Currently loaded: {manifest.GetStep(manifest.CurrentStepIdx).Name}");
        ...
        var alignment = new StyleEnum<TextAnchor>(TextAnchor.MiddleLeft);

        for (int i = 1; i <= manifest.NumSteps; i++)
        {
            var button = new Button();
            buttons.Add(button);
            button.text = $"Load {manifest.GetStep(i).Name}";
            button.style.unityTextAlign = alignment;
            int val = i;
    ...
    {
        manifest.SaveStep();
        EditorUtility.DisplayDialog("Samples", $"Saved Step {manifest.CurrentStepIdx}", "OK");
    }
...
public class Manifest
{
    ...
    {
        return Path.Combine(Application.dataPath, $"../SamplesContent/manifest.json");
    }

    public static string StepPath(int step)
    {
        return Path.Combine(Application.dataPath, $"../SamplesContent/Step_{step}/");
    }
    ...
    {
        var dest = Application.dataPath + $"/../SamplesContent/Step_{stepIdx}.unitypackage";
    ...
    private void ExportPackage(Step step)
    {
        ...
        var dest = Application.dataPath + $"/../SamplesContent/Step_{CurrentStepIdx}.unitypackage";
        Debug.Log(dest);
