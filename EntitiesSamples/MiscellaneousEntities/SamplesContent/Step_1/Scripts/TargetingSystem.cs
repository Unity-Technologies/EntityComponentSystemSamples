...
namespace ClosestTarget
{
    ...
    public partial struct TargetingSystem : ISystem
    {
        ...
        public void OnCreate(ref SystemState state)
        {
            ...
            builder.WithNone<Target, Settings>();
            m_TargetQuery = state.GetEntityQuery(builder);
            
            builder = new EntityQueryBuilder(Allocator.Temp);
            builder.WithAll<LocalTransform, Target>();
            m_QueryKDQuery = state.GetEntityQuery(builder);
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var targetEntities = m_TargetQuery.ToEntityArray(state.WorldUpdateAllocator);
            var targetTransforms =
                m_TargetQuery.ToComponentDataArray<LocalTransform>(state.WorldUpdateAllocator);
            ...
            switch (spatialPartitioningType)
            {
                ...
                case SpatialPartitioningType.KDTree:
                {
                    ...
                    {
                        Tree = tree, TargetEntities = targetEntities, Scratch = default,
                        TargetHandle = targetHandle, LocalTransformHandle = localTransformHandle
                    };
