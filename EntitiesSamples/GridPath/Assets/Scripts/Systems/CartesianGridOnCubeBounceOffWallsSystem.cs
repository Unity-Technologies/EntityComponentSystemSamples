...
#if !ENABLE_TRANSFORM_V1
                    var pos = transform.Position.xz + trailingOffset;
#else
...
#if !ENABLE_TRANSFORM_V1
                        transform.Position = CartesianGridMovement.SnapToGridAlongDirection(transform.Position, prevDir, gridCoordinates, cellCenterOffset);
#else
...
#if !ENABLE_TRANSFORM_V1
                        transform.Position.xyz = math.mul(localToLocal, new float4(transform.Position, 1.0f)).xyz;

                        // Update gridPosition relative to new face.
                        gridCoordinates = new CartesianGridCoordinates(transform.Position.xz + trailingOffsets[nextDir], rowCount, rowCount);
#else
